...
def handle_json_data(json_data,number_all_news):
    ...
    f_tmp_dict=dict()
    number_all_news=0
    for elm in json_data['data']['dataList'][:5]:
        f_tmp_dict=dict()
        ...
        before_wash=("".join(elm['title'].split())).replace('</em>', '')
        f_tmp_dict['title']=''
        for tmp in before_wash:
            if extractchinese.is_legal(tmp):
                f_tmp_dict['title']+=tmp
        before_wash=("".join(elm['abstract'].split())).replace('</em>', '')
        f_tmp_dict['abstract']=''
        for tmp in before_wash:
            if extractchinese.is_legal(tmp):
                f_tmp_dict['abstract']+=tmp
        # print(f_tmp_dict['abstract'])
        # print('====================')
        # for tmp in f_tmp_dict['abstract']:
        #     print(tmp)
...
def baidu_execl(web_content):
    ...
    for elm in web_content:
        ...
        print("%d Done!\n"%(elm['stamp']+1))
        time.sleep(0.25)
    book.save('testexcel.xls')
    
Cookie = get_cookie() #使用selenium模拟登陆获得Cookie

web_content_soup = get_all_news(1,Cookie) #获取给定条件下的全部新闻
json_data = json.loads(web_content_soup.text) #json数据处理为python可理解的数据结构

number_all_news = 0
web_content,number_all_news = handle_json_data(json_data,number_all_news) #清洗数据准备调用百度API
# print(number_all_news)

baidu_execl(web_content) #调用百度API
# print(web_content)
